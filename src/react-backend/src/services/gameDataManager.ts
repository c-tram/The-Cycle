...
import { PlayerGameStats, BoxScore, PlayerTrend, MatchupData, GameInfo } from './boxScoreService';
import { cacheData, getCachedData } from './dataService';

interface GameDataStorage {
  games: { [gameId: string]: BoxScore };
  playerGames: { [playerId: string]: PlayerGameStats[] };
  teamGames: { [teamCode: string]: GameInfo[] };
  dateGames: { [date: string]: GameInfo[] };
  matchups: { [matchupKey: string]: MatchupData };
  lastUpdated: string;
}

class GameDataManager {
  private readonly DATA_DIR = path.join(__dirname, '../../data/games');
  private readonly PLAYER_DATA_DIR = path.join(this.DATA_DIR, 'players');
  private readonly MATCHUP_DATA_DIR = path.join(this.DATA_DIR, 'matchups');
  private readonly TEAM_DATA_DIR = path.join(this.DATA_DIR, 'teams');
  
  private gameData: GameDataStorage;
   ...
   */
  private loadGameData(): GameDataStorage {
    const dataPath = path.join(this.DATA_DIR, 'gameData.json');
  ...
  private saveGameData(): void {
    ...
    try {
      fs.writeFileSync(dataPath, JSON.stringify(this.gameData, null, 2));
   ...
   */
  async storeBoxScore(boxScore: BoxScore): Promise<void> {
    ...
    for (const playerStats of allPlayerStats) {
      ...
      this.gameData.playerGames[playerStats.playerId].sort((a, b) => 
    ...
    await cacheData(`boxscore:${gameId}`, boxScore, 10080); // 7 days
   ...
   */
  getPlayerGameLog(playerId: string, timeframe?: string): PlayerGameStats[] {
   ...
   */
  getPlayerVsTeam(playerId: string, opponentTeam: string): PlayerGameStats[] {
    const playerGames = this.gameData.playerGames[playerId] || [];
   ...
   */
  getBatterVsPitcherHistory(batterId: string, pitcherId: string): PlayerGameStats[] {
    ...
    const commonGameIds = new Set(
      batterGames.map(g => g.gameId).filter(gameId =>
        pitcherGames.some(pg => pg.gameId === gameId)
      )
    );
   ...
   */
  calculateRollingAverages(playerId: string, window: number = 10): any[] {
    const playerGames = this.gameData.playerGames[playerId] || [];
   ...
   */
  getTeamGames(teamCode: string, timeframe?: string): GameInfo[] {
   ...
   */
  getGamesByDateRange(startDate: string, endDate: string): GameInfo[] {
   ...
   */
  getPlayerTrends(playerId: string, timeframe: string = 'last30'): any {
   ...
   */
  getMatchupMatrix(teamCode?: string, opponentTeam?: string): { [key: string]: MatchupData } {
    const matchupMatrix: { [key: string]: MatchupData } = {};
    ...
    const filteredGames = games.filter(game => {
      if (teamCode && opponentTeam) {
        return (
          (game.gameInfo.homeTeam === teamCode && game.gameInfo.awayTeam === opponentTeam) ||
   ...
   */
  private processMatchups(
    batters: PlayerGameStats[], 
    pitchers: PlayerGameStats[],
    gameDate: string,
    matrix: { [key: string]: MatchupData }
  ): void {
    ...
    batters.forEach(batter => {
      ...
      pitchers.forEach(pitcher => {
        ...
        if (estimatedBattersFaced === 0) return;
        ...
        if (totalTeamAtBats === 0 || pitcherBattersFaced === 0) return;
   ...
   */
  getPlayerTrendAnalysis(playerId: string): PlayerTrend {
   ...
   */
  private calculateBattingTrends(recentGames: PlayerGameStats[], olderGames: PlayerGameStats[]): any {
   ...
   */
  private calculatePitchingTrends(recentGames: PlayerGameStats[], olderGames: PlayerGameStats[]): any {
   ...
   */
  private calculateTrendDirection(recent: number, overall: number): string {
   ...
   */
  private calculateHotZones(playerId: string): any {
   ...
   */
  getPlayerVsOpponentAnalysis(playerId: string, opponentTeam: string): any {
    ...
    const playerMatchups = Object.values(allMatchups).filter(
      matchup => matchup.batter === playerId || matchup.pitcher === playerId
    );
   ...
   */
  private calculateTeamVsTeam(team1: string, team2: string): any {
    // Find all games between these teams
    const games = Object.values(this.gameData.games).filter(game => 
      (game.gameInfo.homeTeam === team1 && game.gameInfo.awayTeam === team2) || 
    ...
    const team1Wins = games.filter(game => 
      (game.gameInfo.homeTeam === team1 && game.gameInfo.homeScore > game.gameInfo.awayScore) ||
      (game.gameInfo.awayTeam === team1 && game.gameInfo.awayScore > game.gameInfo.homeScore)
    ).length;
    
    const team2Wins = games.filter(game => 
      (game.gameInfo.homeTeam === team2 && game.gameInfo.homeScore > game.gameInfo.awayScore) ||
      (game.gameInfo.awayTeam === team2 && game.gameInfo.awayScore > game.gameInfo.homeScore)
    ).length;
   ...
   */
  private calculateSuccessMetric(games: PlayerGameStats[], playerId: string, opponentTeam: string): number {
   ...
   */
  private aggregateBattingStats(games: PlayerGameStats[]): any {
    const battingGames = games.filter(g => g.battingStats);
    
    if (battingGames.length === 0) return null;
   ...
   */
  private aggregatePitchingStats(games: PlayerGameStats[]): any {
    const pitchingGames = games.filter(g => g.pitchingStats);
