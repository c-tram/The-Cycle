 ...
 */
async function fetchTeamRosterFromAPI(teamAbbr: string, period: string, statType: string) {
    ...
    try {
      ...
      if (playerResponse.ok) {
        ...
        if (statGroup?.splits?.[0]?.stat) {
          ...
          console.warn(`Failed to get stats for ${playerName}:`, (error as Error).message);
        }
  }
