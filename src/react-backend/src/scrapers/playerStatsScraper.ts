 ...
 */
async function scrapeBattingLeaders(driver: WebDriver): Promise<PlayerStat[]> {
  try {
    // Navigate to MLB batting leaders page
    await driver.get('https://www.mlb.com/stats/');
    
    // Wait for stats table to load
    await driver.wait(until.elementLocated(By.css('.bui-table')), 10000);
    
    // Find all player rows
    const playerRows = await driver.findElements(By.css('.bui-table tbody tr'));
    ...
    for (const row of playerRows) {
      ...
      if (cells.length >= 10) {
        // Get player name
        const nameEl = await cells[0].findElement(By.css('a'));
        const name = await nameEl.getText();
        
        // Get team
        const teamEl = await cells[1].findElement(By.css('span'));
 ...
 */
async function scrapePitchingLeaders(driver: WebDriver): Promise<PlayerStat[]> {
  try {
