 ...
 */
router.get('/players/:playerId/summary', async (req: Request, res: Response) => {
  try {
    const { playerId } = req.params;
 ...
 */
router.get('/players/:playerId/trend-analysis', async (req, res) => {
  try {
    const { playerId } = req.params;
    ...
    res.status(500).json({ 
      success: false,
      error: 'Failed to fetch player trend analysis',
