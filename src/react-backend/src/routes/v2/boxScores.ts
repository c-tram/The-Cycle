 ...
 */
router.get('/games/discover/:date', (req, res) => {
  (async () => {
    ...
    res.json({ date, totalGames: games.length, games });
  })().catch(error => {
 ...
 */
router.get('/games/boxscore/:gameId', (req, res) => {
  (async () => {
 ...
 */
router.get('/players/:playerId/gamelog', (req, res) => {
  (async () => {
    const { playerId } = req.params;
    const { timeframe, limit } = req.query;
    let gameLog = gameDataManager.getPlayerGameLog(playerId, timeframe as string);
    if (limit && !isNaN(Number(limit))) {
      gameLog = gameLog.slice(0, Number(limit));
    }
    res.json({ playerId, timeframe: timeframe || 'all', totalGames: gameLog.length, gameLog });
  })().catch(error => {
 ...
 */
router.get('/players/:playerId/trends', (req, res) => {
  (async () => {
    const { playerId } = req.params;
    const { timeframe = 'last30' } = req.query;
    const trends = gameDataManager.getPlayerTrends(playerId, timeframe as string);
    res.json({ playerId, timeframe, ...trends });
  })().catch(error => {
 ...
 */
router.get('/players/:playerId/rolling-averages', (req, res) => {
  (async () => {
    const { playerId } = req.params;
    const { window = 10 } = req.query;
    const windowSize = Math.max(1, Math.min(50, Number(window))); // Limit between 1-50 games
    const rollingAverages = gameDataManager.calculateRollingAverages(playerId, windowSize);
    res.json({ playerId, window: windowSize, data: rollingAverages });
  })().catch(error => {
 ...
 */
router.get('/players/:playerId/vs-team/:teamCode', (req, res) => {
  (async () => {
    const { playerId, teamCode } = req.params;
    const vsTeamStats = gameDataManager.getPlayerVsTeam(playerId, teamCode.toUpperCase());
    const battingGames = vsTeamStats.filter((g: any) => g.battingStats);
    let aggregateStats = null;
    if (battingGames.length > 0) {
      const totals = battingGames.reduce((acc: any, game: any) => {
    ...
    res.json({ playerId, vsTeam: teamCode.toUpperCase(), totalGames: vsTeamStats.length, aggregateStats, gameLog: vsTeamStats });
  })().catch(error => {
 ...
 */
router.get('/matchups/:batterId/vs/:pitcherId', (req, res) => {
  (async () => {
    const { batterId, pitcherId } = req.params;
    const matchupHistory = gameDataManager.getBatterVsPitcherHistory(batterId, pitcherId);
    let matchupStats = null;
    const battingGames = matchupHistory.filter((g: any) => g.battingStats);
    if (battingGames.length > 0) {
      const totals = battingGames.reduce((acc: any, game: any) => {
        const stats = game.battingStats!;
        return {
    ...
    res.json({ batter: batterId, pitcher: pitcherId, matchupStats, gameHistory: matchupHistory });
  })().catch(error => {
 ...
 */
router.get('/players/:playerId/summary', (req, res) => {
  (async () => {
    const { playerId } = req.params;
    const summary = gameDataManager.getPlayerSummary(playerId);
    if (!summary) {
 ...
 */
router.get('/teams/:teamCode/games', (req, res) => {
  (async () => {
    const { teamCode } = req.params;
    const { timeframe, limit } = req.query;
    let teamGames = gameDataManager.getTeamGames(teamCode.toUpperCase(), timeframe as string);
    if (limit && !isNaN(Number(limit))) {
      teamGames = teamGames.slice(0, Number(limit));
    }
    res.json({ team: teamCode.toUpperCase(), timeframe: timeframe || 'all', totalGames: teamGames.length, games: teamGames });
  })().catch(error => {
 ...
 */
router.get('/games/date-range', (req, res) => {
  (async () => {
    ...
    if (!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(startDate as string) || !/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(endDate as string)) {
      return res.status(400).json({ error: 'Invalid date format. Use YYYY-MM-DD' });
    }
    const games = gameDataManager.getGamesByDateRange(startDate as string, endDate as string);
    res.json({ startDate, endDate, totalGames: games.length, games });
  })().catch(error => {
 ...
 */
router.post('/games/bulk-scrape', (req, res) => {
  (async () => {
    ...
    res.json({ message: 'Bulk scraping initiated', startDate, endDate, forceRefresh, estimatedGames: gameCount, status: 'queued', note: 'This will be implemented with proper job queue system' });
  })().catch(error => {
...
/**
 * GET /api/v2/players/:playerId/trend-analysis
 * Get comprehensive player trend analysis with multiple timeframes and detailed metrics
 */
router.get('/players/:playerId/trend-analysis', (req, res) => {
  (async () => {
    const { playerId } = req.params;
    const trendAnalysis = gameDataManager.getPlayerTrendAnalysis(playerId);
    res.json({ success: true, data: trendAnalysis });
  })().catch(error => {
    console.error('Error fetching player trend analysis:', error);
    res.status(500).json({ success: false, error: 'Failed to fetch player trend analysis', message: error instanceof Error ? error.message : 'Unknown error' });
  });
});

// Helper function to estimate game count
async function estimateGameCount(startDate: string, endDate: string): Promise<number> {
