    ...
    res.json({
      ...
      analytics: {
        ...
        wins: teamGames.filter(g => {
          if (!g.status.isCompleted) return false;
...
router.get('/team/:teamCode', async (req, res) => {
  try {
    ...
    const teamData = await comprehensiveMLBDataService.getComprehensiveTeamData(teamCode);
...
router.get('/scheduler/status', async (req, res) => {
  try {
    ...
    const status = await comprehensiveDataScheduler.getSchedulerStatus();
    res.json({
      success: true,
      data: status,
...
router.post('/scheduler/historical', async (req, res) => {
  try {
    ...
    const jobId = await comprehensiveDataScheduler.startHistoricalDataCollection();
    res.json({
      success: true,
...
router.post('/scheduler/daily-update', async (req, res) => {
  try {
    ...
    const jobId = await comprehensiveDataScheduler.executeDailyUpdate();
    res.json({
      success: true,
...
router.post('/scheduler/enable-analysis', async (req, res) => {
  try {
    ...
    await comprehensiveDataScheduler.enableAnalysisCapabilities();
